...
impl SuggestedRuleView {
    ...
    pub fn add_rule(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let logging_id = rule.logging_id.to_string();
        project_context.update(ctx, |project_context, ctx| {
            if let Err(err) =
                project_context.create_local_global_rule(name, content, Some(logging_id), ctx)
            {
                log::warn!("Failed to create local WarpOCA suggested rule: {err}");
            }
        });
