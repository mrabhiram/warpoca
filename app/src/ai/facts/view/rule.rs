...
use crate::cloud_object::{CloudObject, GenericStringObjectFormat, JsonObjectType, Revision};
...
const ZERO_STATE_TEXT: &str =
    "Add a rule above, or drop Markdown files in ~/Library/Application Support/WarpOCA/rules to apply them across every project.";
#[cfg(target_os = "windows")]
const ZERO_STATE_TEXT: &str = r"Add a rule above, or drop Markdown files in %APPDATA%\WarpOCA\rules to apply them across every project.";
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
const ZERO_STATE_TEXT: &str =
    "Add a rule above, or drop Markdown files in ~/.local/share/WarpOCA/rules to apply them across every project.";
...
impl RuleView {
    ...
    pub fn add_ai_rule(
        ...
        let project_context = ProjectContextModel::handle(ctx);
        project_context.update(ctx, |project_context, ctx| {
            if let Err(err) = project_context.create_local_global_rule(name, content, None, ctx) {
